...
class DFFormat(object):
    ...
    def __str__(self):
        return "DFFormat(%s,%s,%s,%s,%s)" % (self.type, self.name, self.format, self.columns,self.unit_id_to_unit_name)
...
#            print("%s => %s" % (unit_id, label))
            self.unit_id_to_unit_name[elements[1]] = DFUnit(unit_id, label)
